---
name: social-search
description: Search X/Twitter, Hacker News, and Reddit and return structured results.
disable-model-invocation: true
---

Search X, Hacker News, and Reddit unless the user limits the scope. Run the searches in parallel and combine the most relevant results with source links.
...
Use Reddit cookies from the first logged-in browser (Chrome, Firefox, Edge, then Brave), saved to `~/.config/rdt-cli/credential.json` on first use:

```bash
QUERY='your search query'
uvx --from 'git+https://github.com/public-clis/rdt-cli.git' \
  rdt search "$QUERY" -n 20 --compact --json
```

Use `-r <subreddit>` to search one subreddit. Sort with `-s relevance|hot|top|new|comments` and limit time with `-t hour|day|week|month|year|all`.

Read posts from `.data`. Build URLs as `https://www.reddit.com<permalink>`; `url` is the linked page for link posts. Use `--after <cursor>` from the `More:` hint for the next page. Retry once on `rate_limited` or `api_error`.
